...
impl<'a> InternalRedisStore<'a> {
    const KEY_VERSION: &str = "cell_v2_";
...
impl<'a> Store for InternalRedisStore<'a> {
    fn compare_and_swap_with_ttl(
        ...
        let key = self.r.open_key_writable(&Self::versioned_key(key));
        match key.read()? {
            Some(s) => {
                ...
                if !s.is_empty() && s.parse::<i128>()? == old {
    ...
    fn get_with_time(
        ...
        let key = self.r.open_key(&Self::versioned_key(key));
        match key.read()? {
            Some(s) => {
                let n = s.parse::<i128>()?;
